// ===== EMAIL BUILDER (charcode array so CloudFlare cannot find it) =====
function getEmail() {
  var a = [105,110,102,111,64,55,102,101,97,116,104,101,114,115,114,101,116,114,101,97,116,46,99,111,109];
  return a.map(function(c){ return String.fromCharCode(c); }).join('');
}
...
function populateEmails() {
  ...
  document.querySelectorAll('[data-role="email-link"]').forEach(function(el) {
    ...
    el.href = 'mai' + 'lto:' + em + (subj ? '?subject=' + encodeURIComponent(subj) : '');
  });
}
...
function unlock() {
  var input = document.getElementById('accessCode').value.trim().toLowerCase();
  ...
  if (version) {
    ...
    if (version === 'business') {
      navLinks.innerHTML =
        ...
        '<a href="mai' + 'lto:' + em + '?subject=Investment%20Inquiry" class="nav-cta">Request Deck</a>';
    } else {
      navLinks.innerHTML =
        ...
        '<a href="mai' + 'lto:' + em + '?subject=Partnership%20Inquiry" class="nav-cta">Connect</a>';
    }

    populateEmails();
    drawAllCanvases(version);
  } else {
    alert('That code doesn\u2019t match. Please reach out for access.');
...
function drawHeroCanvas(canvasId) {
  ...
  for (var i = 0; i < 7; i++) {
    ...
    ctx.save(); ctx.translate(fx, fy); ctx.rotate(angle + Math.PI / 2); ctx.globalAlpha = 0.7;
    var L = 70, W = 18;
    ctx.beginPath();
    ctx.moveTo(0, L*0.35);
    ...
    bg.addColorStop(0, featherCols[i]+'CC'); bg.addColorStop(0.5, featherCols[i]+'88'); bg.addColorStop(1, featherCols[i]+'33');
    ctx.fillStyle = bg; ctx.fill();
    ctx.beginPath(); ctx.moveTo(0,L*0.33); ctx.lineTo(0,-L*0.62);
    ctx.strokeStyle = featherCols[i]; ctx.lineWidth = 0.8; ctx.globalAlpha = 0.3; ctx.stroke();
    ctx.restore();
    ctx.save(); ctx.globalAlpha = 0.12;
    var grad = ctx.createLinearGradient(fx, fy, cx, cy);
    grad.addColorStop(0, featherCols[i]); grad.addColorStop(1, 'rgba(242,200,75,0)');
...
function drawCTAFeather(canvasId) {
  ...
  ctx.setTransform(DPR, 0, 0, DPR, 0, 0);
  ctx.clearRect(0, 0, S, S);
  ctx.save(); ctx.translate(S/2, S*0.75); ctx.globalAlpha = 0.6;
...
function drawLargeFeather(canvas, col, idx, opacity) {
  ...
  ctx.translate(S/2, H*0.78); ctx.rotate(v[3]*0.5);
  ...
  for (var side=-1;side<=1;side+=2){for(var j=0;j<barbN;j++){
    var frac=j/(barbN-1),ry=L*0.3-frac*(L*0.3+L*0.6),rx=bias*4*(1-frac);
    var barbBase=W*(0.55+0.55*Math.sin(frac*Math.PI))*(0.65+0.35*(1-frac)),barbLen=barbBase*0.85;
...
// ===== INIT =====
(function(){
  function init() { drawSmallLogo('gateLogo'); populateEmails(); }
