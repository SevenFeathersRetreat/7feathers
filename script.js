...
function drawCardFeathers(){
  ...
  function drawLargeFeather(canvas,col,idx,opacity){
    ...
    var L=H*0.88*v[0], W=S*0.38*v[1], bias=v[2];
    ctx.beginPath(); ctx.moveTo(0,L*0.35);
    ...
    var barbN=v[4]+4;
    for(var side=-1;side<=1;side+=2){ for(var j=0;j<barbN;j++){
      var frac=j/(barbN-1); var ry=L*0.3-frac*(L*0.3+L*0.6); var rx=bias*4*(1-frac);
      var barbBase=W*(0.55+0.55*Math.sin(frac*Math.PI))*(0.65+0.35*(1-frac)); var barbLen=barbBase*0.85;
      var bA=side*(0.3+0.5*frac+bias*0.2);
      var ex=rx+Math.sin(bA)*barbLen*side; var ey=ry-Math.cos(bA)*barbLen*0.55;
      var cpx=rx+Math.sin(bA)*barbLen*0.45*side; var cpy=ry-Math.cos(bA)*barbLen*0.3;
      ctx.beginPath(); ctx.moveTo(rx,ry); ctx.quadraticCurveTo(cpx,cpy,ex,ey);
      ctx.lineWidth=(0.4+0.6*(1-frac))*0.85; ctx.globalAlpha=opacity*(0.08+0.15*(1-frac));
      ctx.strokeStyle=side===-1?col.mid:col.base; ctx.stroke();
    }}
    ctx.globalAlpha=opacity;
