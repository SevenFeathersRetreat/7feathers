...
// ═══════════════════════════════════════
var _runningCanvases = {};

function startHeroCanvas(canvasId){
  if(_runningCanvases[canvasId]) return;
  _runningCanvases[canvasId] = true;
...
function openDeck(){
  ...
  var obj     = document.getElementById('deckObject');
  if(!overlay || !obj) return;
  if(!obj.getAttribute('data') || obj.getAttribute('data') === ''){
    obj.setAttribute('data','seven-feathers-deck.pdf');
  }
