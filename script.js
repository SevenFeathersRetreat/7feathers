...
document.addEventListener('keydown',function(e){
  ...
  if(overlay&&overlay.classList.contains('open')){
    if((e.ctrlKey||e.metaKey)&&(e.key==='c'||e.key==='a'||e.key==='p'||e.key==='s')){e.preventDefault();}
  }
});

// Drag prevention on doc overlay
document.addEventListener('DOMContentLoaded',function(){
  var overlay=document.getElementById('docOverlay');
  if(overlay){overlay.addEventListener('dragstart',function(e){e.preventDefault();});}
});
...
(function(){
  function drawNavLogo(){
    ...
    if(!c)return;
    var DPR=Math.min(window.devicePixelRatio||1,2);
    var S=56;c.width=S*DPR;c.height=S*DPR;
  ...
  if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',drawNavLogo);}
  else{drawNavLogo();}
})();
