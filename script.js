...
// PAGE & NAV
// ═══════════════════════════════════════
var CODES=["groundwork7","homecoming7"];
...
function unlock(){
  ...
  if(CODES.indexOf(v)!==-1){
    ...
    buildNavLinks();
    updateNav();
    if(window._startHeroAnim)window._startHeroAnim();
  } else {
...
// ── Deck viewer ──────────────────────────────────────────────────────────────
function openDeck(){
  var o=document.getElementById('deckOverlay');
...
function buildNavLinks(){
  ...
  var links=[
    {label:'The Stay',href:'#b-concept'},
    {label:'The Model',href:'#b-model'},
    {label:'The Ask',href:'#b-market'},
...
function submitContactForm(e){
  ...
  var msg=document.getElementById('cf-message')?document.getElementById('cf-message').value:'';
  var subject=encodeURIComponent('Seven Feathers Partnership Inquiry — '+name);
  var body=encodeURIComponent('Name: '+name+'\nEmail: '+email+'\nRole: '+role+'\n\n'+msg);
  window.location.href='mailto:info@7feathersretreat.com?subject='+subject+'&body='+body;
...
(function(){
  ...
  // Don't start immediately — wait until the page is visible
  window._startHeroAnim=function(){
    startTime=null;
    requestAnimationFrame(update);
  };
})();
...
(function(){
  ...
  function drawLargeFeather(canvas,col,idx,opacity){
    ...
    var ig=ctx.createRadialGradient(-W*0.1,-L*0.05,0,0,-L*0.1,W*0.8);ig.addColorStop(0,col.light);ig.addColorStop(1,'transparent');ctx.fillStyle=ig;ctx.fill();ctx.globalAlpha=opacity;
    var barbN=v[4]+4;
