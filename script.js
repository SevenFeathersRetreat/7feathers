...
function drawCardFeathers(){
  ...
  function drawFeatherOnCard(canvas, col, idx, opacity){
    ...
    ctx.save();
    // anchor point: 78% down the canvas, centered
    ctx.translate(SW/2, SH*0.78);
    var bias=biases[idx]||0;
    ctx.rotate(bias*0.4);
    var L=120, W=28;
    ctx.beginPath(); ctx.moveTo(0,L*0.32);
    ctx.bezierCurveTo(-W*0.25+bias*8,L*0.18,-W*0.85+bias*4,0,-W*0.88+bias*3,-L*0.14);
    ...
    ctx.bezierCurveTo(W*0.8-bias*4,L*0.02,W*0.24-bias*8,L*0.18,0,L*0.32);
    ctx.closePath();
    var bg=ctx.createLinearGradient(0,L*0.32,0,-L*0.64);
    bg.addColorStop(0,col.light+'DD');
    bg.addColorStop(0.2,col.base+'BB');
    bg.addColorStop(0.5,col.mid+'88');
    bg.addColorStop(1,col.dark+'44');
    ...
    for(var side=-1;side<=1;side+=2){
      for(var j=0;j<barbN;j++){
        var frac=j/(barbN-1);
        var ry=L*0.28-frac*(L*0.28+L*0.58);
        ...
        ctx.beginPath(); ctx.moveTo(rx,ry); ctx.quadraticCurveTo(cpx,cpy,ex,ey);
        ctx.lineWidth=0.35+0.55*(1-frac);
    ...
    ctx.beginPath(); ctx.moveTo(bias*4,L*0.30); ctx.lineTo(bias*1.5,-L*0.62);
    ctx.strokeStyle=col.shaft; ctx.lineWidth=1.0; ctx.stroke();
    ctx.restore();
  }

  document.querySelectorAll('canvas.fc-feather-lg').forEach(function(canvas){
    var idx=parseInt(canvas.getAttribute('data-index'),10);
    drawFeatherOnCard(canvas,chakras[idx],idx,0.88);
  });
  document.querySelectorAll('canvas.fc-feather-lg-back').forEach(function(canvas){
    var idx=parseInt(canvas.getAttribute('data-index'),10);
    drawFeatherOnCard(canvas,chakras[idx],idx,0.25);
  });
}
