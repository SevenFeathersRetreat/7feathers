...
function showPage(n){
  ...
  window.scrollTo(0,0);
  var inv=n==='invest';
  ['l1','l2','l3'].forEach(function(id){var e=document.getElementById('nl'+id);if(e)e.style.display=inv?'none':'';});
...
(function(){
  ...
  function Feather(i){
    ...
    // Start widely scattered above and to the sides
  ...
  // Smooth ease with no abrupt transitions
  ...
  function update(ts){
    ...
    for(var i=0;i<feathers.length;i++){
      ...
      if(t<0){
        f.phase='waiting';f.opacity=0;
      }
      else if(t<f.fallDur){
        // Single smooth fall - sway is part of the journey, not a separate phase
        f.phase='falling';
        var p=eSmooth(t/f.fallDur);
        // How much sway is left - fades with the cube of remaining distance
        var sway=(1-p)*(1-p)*(1-p);
        // Position: smooth interpolation with diminishing sway
        f.x=f.sx+(f.tx-f.sx)*p+Math.sin(t*f.swayFreq)*f.swayAmpX*sway;
        f.y=f.sy+(f.ty-f.sy)*p;
        // Rotation: interpolate toward target with diminishing rock
...
(function(){
  ...
  // Per-feather variation: [lengthMul, widthMul, curve, tilt, barbDensity]
  var variations=[
    [1.00, 1.00,  0.04,  0.00, 16], // Crown    - straight, full
    [0.95, 0.95,  0.08,  0.06, 15], // Third Eye - slight curve right
    [1.05, 0.90, -0.06, -0.05, 17], // Throat   - curve left, narrow
    [0.98, 1.10,  0.02,  0.00, 16], // Heart    - wide, full
    [1.02, 0.85,  0.10,  0.08, 14], // Solar    - curved, sparse
    [0.92, 1.05, -0.09, -0.07, 15], // Sacral   - curve left, wide
    [1.08, 0.92,  0.05,  0.03, 18], // Root     - longest, extra barbs
  ];

  function drawCardFeather(canvas, col, idx){
    var S=40, H=56;
    var DPR=Math.min(window.devicePixelRatio||1,2);
    ...
    var L=42*v[0], W=10*v[1], bias=v[2];
  ...
  document.addEventListener('DOMContentLoaded', function(){
    document.querySelectorAll('canvas.fc-feather').forEach(function(canvas){
      ...
      drawCardFeather(canvas, chakras[idx], idx);
    });
    ...
    function drawLargeFeather(canvas, col, idx, opacity){
      ...
      ctx.closePath();
      var bg=ctx.createLinearGradient(0,L*0.35,0,-L*0.64);
      bg.addColorStop(0,col.light+'DD');bg.addColorStop(0.15,col.base+'BB');bg.addColorStop(0.4,col.mid+'99');bg.addColorStop(0.7,col.dark+'66');bg.addColorStop(1,col.dark+'33');
