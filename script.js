...
(function(){
  ...
  var startTime=null,ANIM_DONE=9;
  var replayBtn=document.getElementById('heroReplay');
  var heroText=document.getElementById('heroText');

  window.replayHeroAnim=function(){
    rng=mulberry32(42);
    feathers=chakras.map(function(_,i){return new Feather(i);});
    startTime=null;
    if(replayBtn)replayBtn.classList.remove('show');
    if(heroText){heroText.style.animation='none';heroText.style.opacity='0';
      requestAnimationFrame(function(){heroText.style.animation='heroTextIn 2s ease-out 7s forwards';});}
  };

  function update(ts){
    if(!startTime)startTime=ts;
    var elapsed=(ts-startTime)/1000;
    if(elapsed>ANIM_DONE&&replayBtn&&!replayBtn.classList.contains('show'))replayBtn.classList.add('show');
